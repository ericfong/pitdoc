(function(){
    ...
    _.extend(E, {
        ...
        _get: function(path, cb) {
            path = E.util.pathNormalize(path, true);
            var res = localStorage.getItem( this._dbPath(path) );
            // merge and reduce in here ?
            if (res) {
                if (typeof res == 'string' && res[0] == '{')
                    res = JSON.fromJson(res);
                if (cb)
                    setTimeout(function(){ cb(res) }, 0);
                // for token or url copy, even no in the appcache, can also read
//            } else if (E._appcaches && E._appcaches[path]) {
//                E.ajax(path, cb);
//            } else if (E._acTrees && E._acTrees[path]) {
//                var newPath = path.replace('.tree', '.vT'+E._acTrees[path]+'.tree')
//                E.ajax(newPath, cb);
            } else {
        ...
        _put: function(path, body, cb) {
            ...
            path = E.util.pathNormalize(path, true);
            localStorage.setItem( this._dbPath(path), body);
            if (cb)
                setTimeout(function(){ cb() }, 0);
        },
//        _has: function(path, cb) {
//            var ret = path in localStorage;
//            if (cb) setTimeout(function(){ cb(ret) }, 0);
//        },
//        _del: function(path, cb) {
...
(function(a) {
    ...
    // override by url params
    var m = /deviceDetected=([^&#]+)/.exec(location.search);
    if (m && m[1] && ['mobile', 'tablet', 'desktop'].indexOf(m[1]) >= 0)
        window.deviceType = m[1];
